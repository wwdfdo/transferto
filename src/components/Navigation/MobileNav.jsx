...
const MobileNav = () => {
  ...
  const closeIcon = (
    <CgClose
      className="text-[30px] text-right absolute right-3 top-3 text-black"
      onClick={() => {
        setOpen(!open);
      }}
    />
  ...
  return (
    <div className="flex z-100 justify-between absolute w-11/12  lg:hidden">
      <Logo width="w-[50px]" />
      <div
        className={`lg:hidden items-center   text-[1.4rem] tracking-[0.5rem] font-semibold flex gap-5  flex-col ${
          open
            ? "bg-slate-500 p-10  absolute top-0 right-0  w-[100%]   bg-opacity-95"
            : ""
        }`}
      >
